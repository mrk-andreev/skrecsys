...
pub fn similarity(pui: &Csr, row_scale: &[f64], col_scale: &[f64], k: usize) -> CsrOwned {
    similarity_rows(pui, row_scale, col_scale, k, None)
}

/// [`similarity`] restricted to `rows`, which must be sorted and distinct.
///
/// The result has one row per entry of `rows`, in that order; `None` computes all of
/// them. An incremental fit recomputes the rows a batch can reach in two hops and keeps
/// the rest, which are unchanged entry for entry.
pub fn similarity_rows(
    pui: &Csr,
    row_scale: &[f64],
    col_scale: &[f64],
    k: usize,
