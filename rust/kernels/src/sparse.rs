...
impl CsrOwned {
    ...
    {
        Self::build_indexed(n_rows, |row| row, init, fill)
    }

    /// Build only the rows in `rows`, in that order, as a `rows.len()`-row matrix.
    ///
    /// What an incremental fit needs: a batch changes the similarity rows of the items
    /// it can reach and no others, so the block of those rows is recomputed and spliced
    /// into the stored matrix. `rows` must be sorted and distinct, which is what makes
    /// the result splice-able.
    pub fn build_rows<S, I, F>(rows: &[usize], init: I, fill: F) -> Self
    where
        S: Send,
        I: Fn() -> S + Sync + Send,
        F: Fn(&mut S, usize, &mut Vec<(usize, f64)>) + Sync + Send,
    {
        Self::build_indexed(rows.len(), |at| rows[at], init, fill)
    }

    /// The shared body: `n_out` output rows, where output row `at` is `row_of(at)`.
    fn build_indexed<S, I, F, R>(n_out: usize, row_of: R, init: I, fill: F) -> Self
    where
        S: Send,
        I: Fn() -> S + Sync + Send,
        F: Fn(&mut S, usize, &mut Vec<(usize, f64)>) + Sync + Send,
        R: Fn(usize) -> usize + Sync + Send,
    {
        let n_rows = n_out;
        ...
        let fragments: Vec<Fragment> = (0..n_rows.div_ceil(block))
            ...
            .map_init(init, |state, b| {
                ...
                for at in b * block..stop {
                    let before = entries.len();
                    fill(state, row_of(at), &mut entries);
