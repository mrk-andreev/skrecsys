...
/// `P = (G + l2_reg * I)^-1` as a dense row-major `n x n` matrix.
///
/// `gram` is `n x n` and symmetric, and is left untouched. This is the expensive half of
...
/// Item-item weights `B[i][j] = -P[i][j] / P[j][j]`, with a zero diagonal.
///
/// `p` is the dense row-major, fully populated inverse [`inverse`] returns.
pub fn weights_from_inverse(p: &[f64], n: usize) -> Vec<f64> {
    let mut b = vec![0.0; n * n];
    b.par_chunks_mut(n).enumerate().for_each(|(i, row)| {
        for (j, value) in row.iter_mut().enumerate() {
            if i != j {
                *value = -p[i * n + j] / p[j * n + j];
            }
        }
    });
    b
...
pub fn weights(
    ...
    Ok(weights_from_inverse(&inverse(gram, n, l2_reg, par)?, n))
}
