...
/// training AUC.
///
/// `positives` narrows which stored entries an epoch draws its positives from, by their
/// offset into `r.indices`; `None` draws from all of them, which is the reference's
/// behaviour. The negative is still rejected against the user's whole row of `r`, so a
/// restricted epoch trains on those entries without ever proposing an item the user is
/// already known to have interacted with. That split is what lets an incremental fit
/// learn from one batch while still knowing everything the model has seen.
pub fn fit(r: &Csr, positives: Option<&[usize]>, model: &mut Model, hyper: &Hyper) -> Vec<f64> {
    ...
    let history = (0..hyper.n_iter)
        .map(|epoch| {
            let (correct, skipped) = (0..n_samples)
                ...
                .map(|s| {
                    step(
                        r, positives, &user_of, &users, &items, &biases, hyper, epoch, s,
                    )
                })
...
mod tests {
    ...
    fn fit_dense(dense: &[Vec<f64>], hyper: &Hyper) -> Fitted {
        fit_dense_over(dense, None, hyper)
    }

    fn fit_dense_over(dense: &[Vec<f64>], positives: Option<&[usize]>, hyper: &Hyper) -> Fitted {
    ...
    /// Naming every entry is the same fit as naming none, which is the guarantee that
    /// lets `partial_fit` share this kernel with `fit` rather than fork it.
    #[test]
    fn every_entry_as_a_positive_is_the_unrestricted_fit() {
        let dense = pseudo_random_dense(12, 9);
        let hyper = hyper(4, 6);
        let all: Vec<usize> = (0..Owned::from_dense(&dense).csr().indices.len()).collect();
        let restricted = fit_dense_over(&dense, Some(&all), &hyper);
        let full = fit_dense(&dense, &hyper);
        assert_eq!(restricted.user_factors, full.user_factors);
        assert_eq!(restricted.item_factors, full.item_factors);
        assert_eq!(restricted.item_bias, full.item_bias);
        assert_eq!(restricted.history, full.history);
    }

    /// A restricted epoch leaves alone the users none of its entries belong to, which is
    /// what makes it an update of the batch rather than a fit of a truncated matrix.
    #[test]
    fn a_restricted_fit_moves_only_the_users_it_draws() {
        let dense = pseudo_random_dense(10, 8);
        let hyper = hyper(3, 4);
        let owned = Owned::from_dense(&dense);
        let r = owned.csr();
        // Entries of the last two users only.
        let first = r.indptr[r.n_rows - 2];
        let restricted: Vec<usize> = (first..r.indices.len()).collect();
        let fitted = fit_dense_over(&dense, Some(&restricted), &hyper);
        ...
        assert!((r.n_rows - 2..r.n_rows).any(moved));
    }
