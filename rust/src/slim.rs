...
pub fn similarity(r: &Csr, enet: &ElasticNet, k: usize) -> (CsrOwned, usize) {
    ...
    let weights = CsrOwned::build(
        n,
        || Solver::new(n),
        |solver, j, out| {
            if !solver.solve(&g, n, j, l1, l2, enet) {
...
/// `y += a * x`, the one dense operation in the inner loop.
///
/// Coordinate descent spends nearly all its time here, over contiguous slices with no
/// indirection, which is the shape a compiler vectorizes well. The default x86-64
/// baseline is only SSE2, so the wheels would otherwise run this two lanes at a time;
/// `multiversion` compiles a clone per feature set and picks one on the first call.
/// Targets whose vector support is already in the baseline, such as aarch64 with NEON,
/// get a single clone and dispatch straight to it.
///
/// AVX-512 is deliberately absent: it adds little to an operation this close to memory
/// bound, and `avx512f` only became a stable target feature in Rust 1.89, so naming it
/// would raise the compiler this crate needs for no measured gain.
